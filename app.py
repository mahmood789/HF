...
# ─── CORS so ChatGPT Actions or other front-ends can call us ─────────
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def get_lab_result(
    test: str = Query(
        ...,
        enum=["ntprobnp", "creatinine", "potassium", "egfr", "hemoglobin", "ferritin"],
    )
):
    values = {
...
def get_imaging_result(
    ...
    return {"url": f"{base}/{fname}", "caption": f"{type} image ({stage})"}
...
@app.get("/diary")
def get_patient_diary(stage: str = Query(..., enum=list(diary_bank.keys()) + ["initial"])):
...
def record_decision(action: str, outcome: str = Query(..., enum=["correct", "incorrect", "partial"])):
    # Create and append the new decision entry
    entry = {
    ...
    # Recalculate XP from all correct outcomes
    xp = sum(2 for d in log if d["outcome"] == "correct")

    # Return a minimal payload to avoid large responses
    return {
        "message": f"Decision recorded: {action} ({outcome})",
        "latest": entry,
        "xp": xp,
    }
