...
# Allow CORS for any front-end (ChatGPT Actions, etc.)
app.add_middleware(
  CORSMiddleware,
  allow_origins=["*"],
  allow_methods=["*"],
...
# In-memory diary entries by stage
diary_bank = {
  "after_echo": {
...
# In-memory log for decisions (scoring)
log: list[dict] = []

# 1. /lab – Return value & interpretation
@app.get("/lab")
def get_lab_result(
  test: str = Query(..., enum=[
    "ntprobnp","creatinine","potassium","egfr","hemoglobin","ferritin"
  ])
):
  values = {
...
# 2. /image – Return static or hosted images (CXR, Echo, ECG)
@app.get("/image")
...
# 3. /diary – Return patient’s diary entry & mood
@app.get("/diary")
...
# 4. /decision – Log decision action/outcome
@app.post("/decision")
def record_decision(
  action: str,
  outcome: str = Query(..., enum=["correct","incorrect","partial"])
):
  log.append({
...
# 5. /trend – Return static trend chart URL & caption
@app.get("/trend")
def get_trend_chart(
  metric: str = Query(..., enum=["weight","ntprobnp","potassium","creatinine"])
):
...
# 6. /summary – Return XP total, decisions, and summary
@app.get("/summary")
